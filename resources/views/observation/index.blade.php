                ...
                <div class="box-antrian">
                    ...
                    <div class="box-antrian-header d-flex justify-content-between align-items-center mb-3">
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            ...
                            <a href="{{ route('observation.create') }}" class="btn-tambah-pasien"
                           ...
                           ">
                            ...
                            <form action="{{ route('observation.index') }}" method="GET"
                                class="form-pencarian d-flex gap-2 mt-3 mt-md-0">
                                <input type="text" name="search" placeholder="Cari Nama Pasien"
                                    class="input-pencarian nama form-control form-control-sm"
                                    value="{{ request('search') }}" />

                                <button type="submit" class="btn-cari btn btn-secondary btn-sm">
                                    Cari
                                </button>
                            </form>
                        </div>
                        ...
                        <tbody>
                            @forelse($observations as $index => $obs)
                                <tr>
                                    ...
                                    {{-- Nomor Observasi --}}
                                    <td>{{ 'OB-' . \Carbon\Carbon::parse($obs->observed_at)->format('Ymd') . '-' . str_pad($obs->id, 3, '0', STR_PAD_LEFT) }}
                                    </td>

                                   <td>{{ $obs->queue?->patient?->name ?? '-' }}</td>
